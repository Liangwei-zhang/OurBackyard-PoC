...
window._p1p2_sendChannelMsg = function() {
  ...
  _renderChannelMsgs();
  // 1. DataChannel broadcast — fastest, reaches directly-connected peers
  if (window.mesh && window.mesh._flood) window.mesh._flood(msg);
  // 2. WebSocket relay (legacy server.py dev mode)
  if (window.ws && window.ws.readyState === WebSocket.OPEN) {
    try { window.ws.send(JSON.stringify(msg)); } catch(e) {}
  }
  // 3. Nostr broadcast — reaches peers without an active DataChannel.
  // Uses KIND_CHAT with no 'target' tag so ALL channel subscribers receive it,
  // including peers on different H3 cells (ob-global-v1 fallback).
