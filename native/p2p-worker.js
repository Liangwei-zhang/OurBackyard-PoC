...
async function handleSync(data) {
  const { localHashes, remoteHashes, requestId } = data;
...
async function handleCompute(data) {
  const { operation, payload, requestId } = data;
...
const P2PWorker = {
  ...
  handleMessage(msg) {
    const { type, messageId, requestId } = msg;
    const key = messageId !== undefined
      ? `${type}:${messageId}`
      : (requestId !== undefined ? `${type}:${requestId}` : type);
  ...
  async broadcast(topic, message) {
    ...
    return new Promise((resolve) => {
      const callbackKey = `broadcasted:${id}`;
      this.callbacks.set(callbackKey, () => {
        resolve({ success: true, messageId: id });
      });
  ...
  async sync(localHashes, remoteHashes) {
    return new Promise((resolve) => {
      ...
      const callbackKey = `sync_result:${requestId}`;
      this.callbacks.set(callbackKey, (msg) => {
        resolve(msg.missing);
      });

      this.send('sync', { localHashes, remoteHashes, requestId });
  ...
  async compute(operation, payload) {
    return new Promise((resolve) => {
      const requestId = ++this.messageId;
      const callbackKey = `compute_result:${requestId}`;
      
      this.callbacks.set(callbackKey, (msg) => {
        resolve(msg.result);
      });
      
      this.send('compute', { operation, payload, requestId });
