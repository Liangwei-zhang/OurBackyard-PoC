...
class OurBackyardMesh extends EventTarget {
  ...
  async sendChat(toPeerId, text, itemId = null, media = null, _overrideMsg = null) {
    ...
    if (dc?.readyState === 'open') {
      ...
      if (msg.mediaData && typeof msg.mediaData === 'string' && msg.mediaData.startsWith('data:')) {
        ...
        await this.sendChatMedia(toPeerId, blob, {
          id:        msg.id,
          from:      msg.from,
          to:        msg.to,
          ts:        msg.ts,
        ...
        console.log('[Mesh] Chat media sent via BLOB_STREAM to', toPeerId.slice(0, 8));
      } else {
      ...
      if (msg.mediaData && msg.mediaData.length > 32000) {
        // Too large for Nostr — queue for when DataChannel opens
        await this._storeDeadDrop(toPeerId, msg);
        this.onChat?.({ ...msg, direction: 'out', status: 'queued' });
        console.log('[Mesh] Media queued for DC delivery (too large for Nostr):', toPeerId.slice(0,8));
      } else {
        try {
  ...
  async _handleChatMsg(fromPeerId, envelope) {
    ...
    if (msg.id && this.db?.chatMessages) {
  ...
  async sendChatMedia(toPeerId, blob, meta) {
    ...
    // Store outgoing message locally + fire onChat so sender sees their own bubble immediately
    const dataUrl = URL.createObjectURL(blob);
    const outMsg  = { ...msg, mediaData: dataUrl, direction: 'out' };
    await this.db?.chatMessages?.put({ ...msg, direction: 'out' }).catch(() => {});
    this.onChat?.({ ...outMsg });
  ...
  _startImgTransfer(fromPeerId, msg) {
    // Legacy IMG_HEADER path disabled — all image transfers now use BLOB_STREAM.
    // Keeping as no-op for backwards compatibility with old nodes.
  }

  _handleImgChunk(fromPeerId, ab) {
    // Legacy binary chunk path disabled.
    // Binary data is now exclusively routed through _routeBinaryChunk (BLOB_STREAM).
    // Dropping this data is safe: the sender will retry via BLOB_REQ.
  }

  _finishImgTransfer(fromPeerId, msg) {
    // Legacy IMG_END path disabled.
  }
