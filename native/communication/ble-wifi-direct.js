...
const BLEDiscovery = {
  ...
  async startAdvertising(peerId, data = {}) {
    ...
    try {
      ...
      this.bleService = await this.server.getPrimaryService('ourbackyard');
      
      // Create characteristics for data exchange
      const peerIdChar = await this.bleService.getCharacteristic('peerId');
      const dataChar = await this.bleService.getCharacteristic('data');
  ...
  async startScanning() {
    ...
    try {
      ...
      const service = await server.getPrimaryService('ourbackyard');
      
      // Read peer ID
      const peerIdChar = await service.getCharacteristic('peerId');
  ...
  async connect(peerId) {
    ...
    try {
      ...
      const service = await server.getPrimaryService('ourbackyard');
      const dataChar = await service.getCharacteristic('data');
