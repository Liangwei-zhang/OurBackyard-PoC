...
class NostrSignaling {
  ...
  constructor({ peerId, userId = null, h3Cell, privateKey, onSignal, onPeerAnnounce, onChat }) {
    this.peerId       = peerId;       // 本节点 ID
    this.userId       = userId || peerId; // 稳定用户ID（跨浏览器一致）
  ...
  _subscribe(ws) {
    ...
    // Global channel constant — all nodes subscribe to this regardless of location.
    // Ensures nodes with no GPS (h3Cell='default') still find other peers.
    const GLOBAL_CHANNEL = 'ob-global-v1';

    // Sub 1: geo-cell channel (signals, announces, local chat)
    const cells = [this.channelCell];
    if (this.channelCell !== GLOBAL_CHANNEL) cells.push(GLOBAL_CHANNEL);
    const geoFilter = {
      kinds: [NostrSignaling.KIND_SIGNAL, NostrSignaling.KIND_ANNOUNCE, NostrSignaling.KIND_CHAT],
      '#h': cells,
      since,
    };
    ws.send(JSON.stringify(['REQ', this.subId, geoFilter]));

    // Sub 2: direct DMs addressed to this peer (#target tag).
    const dmFilter = {
      kinds: [NostrSignaling.KIND_CHAT, NostrSignaling.KIND_SIGNAL],
      '#target': [this.peerId, this.userId],
      since,
    };
    const dmSubId = this.subId + '_dm';
    ws.send(JSON.stringify(['REQ', dmSubId, dmFilter]));
  }
  ...
  _handleRelayMsg(url, raw) {
    ...
    const senderUserId = this._getTag(event, 'user');
    ...
    if (event.kind === NostrSignaling.KIND_ANNOUNCE && senderPeerId) {
      ...
      try {
        ...
        this.onPeerAnnounce?.(senderPeerId, {
          ...meta,
          userId: meta?.userId || senderUserId || senderPeerId,
        });
      } catch {}
      return;
    }
    ...
    if (event.kind === NostrSignaling.KIND_CHAT) {
      try {
        ...
        // Community broadcast: CHANNEL_MSG with no target = goes to all channel subscribers
        if (msg && msg.type === 'CHANNEL_MSG' && !targetPeerId) {
          if (typeof window !== 'undefined' && typeof window.handleMessage === 'function') {
            try { window.handleMessage(event.content); } catch {}
          }
          return;
        }
        // Direct chat: only process messages addressed to this peer
        if (targetPeerId && targetPeerId !== this.peerId && targetPeerId !== this.userId) return;
        if (senderPeerId && msg) {
          // Attach the Nostr event ID so the mesh can dedup across relay re-deliveries
          msg._nostrEventId = event.id;
    ...
    if (event.kind === NostrSignaling.KIND_SIGNAL) {
      // Normal signal: only process messages addressed to this peer
        if (targetPeerId && targetPeerId !== this.peerId && targetPeerId !== this.userId) return;
  ...
  async sendSignal(targetPeerId, signal) {
    ...
    if (this.channelCell !== GLOBAL_CHANNEL) tags.push(['h', GLOBAL_CHANNEL]);
  ...
  /**
   * Broadcast a community CHANNEL_MSG to all peers on both geo + global channels.
   * Uses KIND_CHAT with no 'target' tag so every channel subscriber receives it.
   */
  async broadcastCommunity(msg) {
    const GLOBAL = 'ob-global-v1';
    const tags = [['h', this.channelCell], ['peer', this.peerId], ['user', this.userId]];
    if (this.channelCell !== GLOBAL) tags.push(['h', GLOBAL]);
    const event = await this._buildEvent(
      NostrSignaling.KIND_CHAT,
      JSON.stringify(msg),
      tags
  ...
  async announce(meta = {}) {
    const GLOBAL_CHANNEL = 'ob-global-v1';
    // Tag both the geo-cell and the global channel so peers on either channel see us
    const hTags = [['h', this.channelCell], ['peer', this.peerId], ['user', this.userId]];
    if (this.channelCell !== GLOBAL_CHANNEL) hTags.push(['h', GLOBAL_CHANNEL]);
    const event = await this._buildEvent(
