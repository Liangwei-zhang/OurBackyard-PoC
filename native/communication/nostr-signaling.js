...
class NostrSignaling {
  ...
  async broadcastCommunity(msg) {
    ...
    const tags = [['h', this.channelCell], ['peer', this.peerId]];
  ...
  async announce(meta = {}) {
    ...
    const hTags = [['h', this.channelCell], ['peer', this.peerId]];
    if (this.channelCell !== GLOBAL_CHANNEL) hTags.push(['h', GLOBAL_CHANNEL]);
    const event = await this._buildEvent(
      NostrSignaling.KIND_ANNOUNCE,
      JSON.stringify({ peerId: this.peerId, h3Cell: this.h3Cell, ts: Date.now(), ...meta }),
      hTags
    );
    this._publish(event);
  }
